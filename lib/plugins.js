...
export function clientAssets(plugins, services = {}) {
  ...
  for (const plugin of plugins) {
    ...
    // An admin module is published the same way but injected nowhere: the
    // admin imports it on demand when its screen is opened (§9 admin surface).
    const adminFile = plugin.manifest.admin?.js;
    if (adminFile) {
      const from = path.join(plugin.dir, adminFile);
      if (!fs.existsSync(from)) throw new ContentError(`plugins/${plugin.name}/plugin.json`, null, `declares admin js "${adminFile}" but the file doesn't exist`);
      copies.push({ from, to: `plugins/${plugin.name}/${adminFile}` });
    }
  }
...
/**
 * Admin screens contributed by enabled plugins, for api/site.json.
 *
 * The admin builds its sidebar and routes from this instead of hardcoding a
 * link per feature, so a plugin can add a screen without touching engine-owned
 * files (§14.1 ownership) — which is what a site owner needs, since `admin/`
 * is replaced wholesale on upgrade.
 *
 * @returns {{plugin: string, module: string, id: string, label: string}[]}
 */
export function adminScreens(plugins) {
  const screens = [];
  for (const plugin of plugins) {
    const admin = plugin.manifest.admin;
    if (!admin?.js || !Array.isArray(admin.screens)) continue;
    for (const screen of admin.screens) {
      if (!screen?.id || !screen?.label) {
        throw new ContentError(`plugins/${plugin.name}/plugin.json`, null, 'each admin.screens entry needs an "id" and a "label"');
      }
      screens.push({
        plugin: plugin.name,
        module: `/plugins/${plugin.name}/${admin.js}`,
        id: screen.id,
        label: screen.label,
      });
    }
