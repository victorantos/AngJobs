...
/** Build an item's URL from its collection's pattern. "index" maps to the pattern root. A data-only collection (no urlPattern) has no URL. */
...
export function makeItem(source, { file, slug, collection, def, language = 'en' }) {
  ...
  const item = { ...data, slug, url: def.urlPattern ? urlFor(def.urlPattern, slug) : null, file, collection, body };
...
export function validateConfig(config) {
  ...
  // i18n (§5.4) activates only when languages lists 2+ codes incl. the default.
  config.site.languages ||= [];
  const languages = config.site.languages;
  if (!Array.isArray(languages) || !languages.every((l) => typeof l === 'string' && /^[a-z]{2,3}(-[a-z0-9]{2,8})?$/.test(l))) {
    throw new ContentError(file, null, '"site.languages" must be a list of lowercase language codes like ["en", "fr"]');
  }
  if (languages.length && !languages.includes(config.site.language)) {
    throw new ContentError(file, null, `"site.languages" must include the default language "${config.site.language}" — e.g. ["${config.site.language}", …]`);
  }
  config.collections ||= {};
  config.plugins ||= [];
  config.services ||= {};
  ...
  for (const [name, def] of Object.entries(config.collections)) {
    ...
    // A data-only collection ("render": false) is scanned, validated, sorted,
    // admin-editable and exposed to templates as collections.<name>, but emits
    // no pages — so it needs no urlPattern, template, or list config. Use it
    // for repeated home-page sections (feature cards, FAQ, testimonials…).
    def.render ??= true;
    if (def.render) {
      if (!def.urlPattern?.startsWith('/') || !def.urlPattern.includes(':slug')) throw new ContentError(file, null, `${where} needs a "urlPattern" starting with "/" and containing ":slug" — e.g. "/${name}/:slug/" (or set "render": false for a data-only collection)`);
      if (!def.template) throw new ContentError(file, null, `${where} is missing "template" — the theme template name to render items with (or set "render": false for a data-only collection)`);
