System.register(['angular2/core', '../jobs/jobs.service', 'angular2/router'], function(exports_1, context_1) {
    "use strict";
    var __moduleName = context_1 && context_1.id;
    var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
        var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
        if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
        else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
        return c > 3 && r && Object.defineProperty(target, key, r), r;
    };
    var __metadata = (this && this.__metadata) || function (k, v) {
        if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
    };
    var core_1, jobs_service_1, router_1;
    var JobDetail;
    return {
        setters:[
            function (core_1_1) {
                core_1 = core_1_1;
            },
            function (jobs_service_1_1) {
                jobs_service_1 = jobs_service_1_1;
            },
            function (router_1_1) {
                router_1 = router_1_1;
            }],
        execute: function() {
            JobDetail = (function () {
                function JobDetail(_jobsService, _routeParams, _router) {
                    var _this = this;
                    this._jobsService = _jobsService;
                    this._routeParams = _routeParams;
                    this._router = _router;
                    var id = +this._routeParams.get('id');
                    _jobsService.getJobDetail(id)
                        .subscribe(function (res) { return _this.jobPost = res; });
                }
                JobDetail = __decorate([
                    core_1.Component({
                        selector: 'job-detail',
                        templateUrl: './app/jobs/job-detail.html',
                        directives: [],
                        inputs: ['jobpost']
                    }), 
                    __metadata('design:paramtypes', [jobs_service_1.JobsService, router_1.RouteParams, router_1.Router])
                ], JobDetail);
                return JobDetail;
            }());
            exports_1("JobDetail", JobDetail);
        }
    }
});
//# sourceMappingURL=jobdetail.js.map